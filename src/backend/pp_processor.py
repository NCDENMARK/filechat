import os
import hashlib
from pathlib import Path
from typing import List, Dict
from pptx import Presentation

class PPProcessor:
    def __init__(self):
        # Keep track of files we've already processed
        self.processed_files = set()
    
    def scan_directory(self, directory_path: str) -> List[str]:
        """Find all PowerPoint files in a given folder"""
        pp_files = []  # List to store found PowerPoint paths
        path = Path(directory_path)  # Convert string to Path object
        
        # Check if the directory exists and is actually a directory
        if path.exists() and path.is_dir():
            # Find all .pptx files (case-insensitive)
            for pp_path in path.glob("*.[pP][pP][tT][xX]"):
                pp_files.append(str(pp_path))  # Add full path to list
            # Also check for older .ppt files
            for pp_path in path.glob("*.[pP][pP][tT]"):
                # Only add if it's not already in the list (avoid duplicates)
                path_str = str(pp_path)
                if not path_str.endswith('.pptx') and not path_str.endswith('.PPTX'):
                    pp_files.append(path_str)
        
        return pp_files  # Return list of PowerPoint file paths
    
    def extract_text(self, pp_path: str) -> Dict:
        """Pull all text out of a PowerPoint presentation"""
        try:
            # Open the PowerPoint presentation
            prs = Presentation(pp_path)
            text = ""  # String to accumulate all text
            
            # Extract text from each slide
            for slide_num, slide in enumerate(prs.slides, 1):
                text += f"\n\n--- Slide {slide_num} ---\n"
                
                # Extract text from all shapes in the slide
                for shape in slide.shapes:
                    if hasattr(shape, "text") and shape.text.strip():
                        text += f"{shape.text}\n"
                    
                    # Also extract text from tables
                    if shape.has_table:
                        table = shape.table
                        for row in table.rows:
                            for cell in row.cells:
                                if cell.text.strip():
                                    text += f"{cell.text}\n"
            
            # Create a unique ID for this file using MD5 hash
            file_id = hashlib.md5(pp_path.encode()).hexdigest()
            
            # Count slides
            slide_count = len(prs.slides)
            
            # Return all the extracted information
            return {
                "file_id": file_id,  # Unique identifier
                "file_path": pp_path,  # Full path to file
                "file_name": Path(pp_path).name,  # Just the filename
                "text": text,  # All extracted text
                "slide_count": slide_count,  # Number of slides
                "status": "success"  # Extraction worked
            }
        except Exception as e:
            # If something goes wrong, return error info
            return {
                "file_path": pp_path,
                "file_name": Path(pp_path).name,
                "status": "error",
                "error": str(e)  # What went wrong
            }
    
    def split_into_chunks(self, text: str, chunk_size: int = 500, overlap: int = 50) -> List[str]:
        """Break long text into smaller, overlapping pieces"""
        words = text.split()  # Split text into individual words
        chunks = []  # List to store text chunks
        
        # Move through the text in steps of (chunk_size - overlap)
        for i in range(0, len(words), chunk_size - overlap):
            # Take chunk_size words starting from position i
            chunk = ' '.join(words[i:i + chunk_size])
            if chunk:  # Only add non-empty chunks
                chunks.append(chunk)
        
        return chunks